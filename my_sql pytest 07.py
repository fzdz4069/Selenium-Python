# Testcase 7: Opening an HTML file with chromedriver and analyzing which links inside the file are working
...
cursor.execute("select count(number) from fz_html")
for col in cursor:
    condition = len(link_titles) == int(col[0])
    record_test("7", "off-line HTML test", "Testing links inside the file", condition)
